import logging
from srt_labeler.logger import Logger, STREAM_FORMAT, FILE_FORMAT
...
def test_logger_stream_formatting(caplog):
    ...
    assert STREAM_FORMAT in logger.handlers[0].formatter._fmt  # pyright: ignore[reportOptionalMemberAccess, reportOperatorIssue]
...
def test_logger_file_formatting(tmp_path):
    ...
    assert FILE_FORMAT in file_handler.formatter._fmt  # pyright: ignore[reportOptionalMemberAccess, reportOperatorIssue]
    log_file = tmp_path / 'test.log'
